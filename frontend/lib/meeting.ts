...
export type Requirement = { confidence?: number | null; priority?: Priority | null; role?: string; needs_clarification?: boolean; id: string; title: string; description: string; category: 'functional' | 'nonfunctional'; source: number | null };
...
export function confidenceLabel(value?: number | null): string {
  return value == null ? 'Не оценена' : `${Math.round(value * 100)}%`;
}
  ...
  return [`# Техническое задание\n\n${meeting.title}`, 'Статус: черновик. Требует проверки и согласования.',
    ...ANALYSIS_SECTIONS.map(section => `## ${section.title}\n\n` + (items.filter(item => item.kind === section.key).map((item, i) => {
      ...
      return title + (item.description ? `\n   ${item.description}` : '')
        + `\n   Уверенность анализа: ${confidenceLabel(item.confidence)}`
...
export function parseMeeting(value: unknown): Meeting {
  ...
  const confidence = (v: unknown) => v == null || (number(v) && v <= 1);
  ...
  if (!Array.isArray(requirements) || !requirements.every(r => r && string(r.id) && string(r.title) && string(r.description) && confidence(r.confidence) && ['functional','nonfunctional'].includes(r.category) && source(r.source) && (r.role === undefined || string(r.role)) && (r.needs_clarification === undefined || typeof r.needs_clarification === 'boolean'))) return fail();
  ...
  if (m.analysis !== undefined) {
    if (!Array.isArray(m.analysis) || !m.analysis.every(item => item && string(item.id) && item.id.length > 0
      ...
      && confidence(item.confidence) && string(item.title) && item.title.trim().length > 0 && string(item.description) && source(item.source)
