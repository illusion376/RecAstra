...
flowchart TB
    ...
    subgraph BE["backend · FastAPI"]
        ...
        STORE["Storage (интерфейс)<br/>память + файлы"]
    end
...
| Хранилище | память (встречи), файлы (аудио, документы) | реализация интерфейса `Storage` |
...
- **Встречи живут в памяти** до перезапуска сервера. Аудио и экспортированные
  документы лежат на диске и переживают его. Хранилище спрятано за интерфейсом
  `Storage` — подключение PostgreSQL не затрагивает ни конвейер, ни ручки.
- **Авторизация простая.** Регистрация и вход по почте и паролю, все ручки
  закрыты токеном, но рабочее пространство у всех пользователей общее:
  встречи и документы не привязаны к владельцу, ролей и восстановления
  пароля нет.
...
backend/          FastAPI
  app/
    ...
    storage/      интерфейс хранилища и реализация в памяти
